...
chatSocket.onmessage = function(e) {
    ...
    let html = '<div class="bg-white shadow rounded-md  -mx-2 lg:mx-0 py-2 px-2 mb-2 flex svg-messages-show"><div><a href="/profile/'
        ...
        html += data.username + '">' + data.username + '</a></div><div>' + data.date_added + '</div>'
        if (data.username == userName){
            html += '<div class="svg-messages-hide"><form action="' + delete_message_url + '" method="POST">' + '<input type="hidden" name="csrfmiddlewaretoken" value="'
            html += csrf_token + '">' + '<input type="hidden" value="' + data.message_id + '" name="message_id"><button>'
            ...
            html += '</svg></button></form></div>'
        }
        html += '</div><div>' + data.message + '</div></div></div>'
