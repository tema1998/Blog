...
class ChatViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in_GET(self):
        response = self.client.get(reverse('chat', kwargs={'chat_id': self.chat.id}))
        self.assertRedirects(response, '/signin?next=/chats/1/')

    def test_logged_in_uses_correct_template_GET(self):
        response = self.authorized_client_in_chat.get(reverse('chat', kwargs={'chat_id': self.chat.id}))
    ...
    def test_redirect_if_user_not_in_chat_GET(self):
        response = self.authorized_client_not_in_chat.get(reverse('chat', kwargs={'chat_id': self.chat.id}))
...
class DeleteMessageTest(TestCase):
    ...
    def setUp(self):
        ...
        self.chat.members.add(self.user2_in_chat)
        self.chat.save()
        self.message = Message.objects.create(chat=self.chat, user=self.user1_in_chat, user_profile=self.profile1,
                                              content='content')

    def test_user1_delete_his_message_POST(self):
        redirect_url = reverse('chat', kwargs={'chat_id': self.chat.id})
        response = self.auth_user1_in_chat.post(reverse('delete-message'), data={
...
class DeleteChatTest(TestCase):
    ...
    def test_user1_delete_his_chat_with_user2_POST(self):
        response = self.auth_user1_in_chat.post(reverse('delete-chat'), data={
...
class ClearChatTest(TestCase):
    ...
    def setUp(self):
        ...
        self.chat.members.add(self.user2_in_chat)
        self.chat.save()
        self.message1 = Message.objects.create(chat=self.chat, user=self.user1_in_chat, user_profile=self.profile1,
                                               content='content')
        self.message2 = Message.objects.create(chat=self.chat, user=self.user2_in_chat, user_profile=self.profile2,
                                               content='content123')

    def test_user1_clear_chat_with_user2_POST(self):
        response = self.auth_user1_in_chat.post(reverse('clear-chat'), data={
            'chat_id': self.chat.id,
        })
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/chats/')
