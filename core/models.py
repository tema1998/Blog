...
class Profile(models.Model):
    ...
    location = models.CharField(
        max_length=100, verbose_name="Location", blank=True
    )
...
class Post(models.Model):
    ...
    id = models.UUIDField(
        verbose_name="Post ID", primary_key=True, default=uuid.uuid4
    )
    ...
    image = models.ImageField(
        verbose_name="Post image", upload_to="post_images"
    )
    caption = models.TextField(
        verbose_name="Caption", max_length=1000, blank=True
    )
    created_at = models.DateTimeField(
        verbose_name="Date of creation", default=datetime.now
    )
    no_of_likes = models.IntegerField(
        default=0, verbose_name="Number of likes"
    )
    comments_status = models.BooleanField(
        default=True,
        verbose_name="Comments(uncheck the box to turn off the comments)",
    )
...
class CommentLikes(models.Model):
    ...
    comment = models.ForeignKey(
        PostComments, verbose_name="Comment", on_delete=models.CASCADE
    )
